...
function useUpdateMemberRole() {
  ...
  return useMutation({
    mutationFn: async ({ memberId, role, projectId }: { memberId: string; role: ProjectRole; projectId: string }) => {
      void projectId;
...
export default function ProjectMembersPage() {
  ...
  return (
    <div className="max-w-2xl px-6 py-4">
      <h1 className="text-lg font-semibold text-content-primary mb-4">Project Members</h1>

      {/* Current members */}
      <div className="space-y-2 mb-6">
        {projectMembers?.map((member) => (
          <div key={member.id} className="flex items-center justify-between border border-border-subtle rounded-md p-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[13px] text-content-primary">{member.user.name}</span>
            </div>
            <select
              value={member.role}
              onChange={(e) =>
      ...
      {nonMembers.length > 0 && (
        <div>
          <h2 className="text-sm font-medium text-content-primary mb-2">Add Members</h2>
          <div className="flex flex-wrap gap-2">
            {nonMembers.map((m) => (
              <button
                key={m.id}
                onClick={() => addMember.mutate({ projectId, userId: m.id, role: 'member' })}
      ...
      <div className="mt-6 border-t border-border-subtle pt-4">
        <h3 className="text-[11px] font-medium text-content-muted uppercase mb-2">Role Permissions</h3>
        <div className="space-y-1">
          {ROLES.map((r) => (
            <div key={r.value} className="flex items-center gap-2">
              <span className="text-[12px] font-medium text-content-primary w-16">{r.label}</span>
