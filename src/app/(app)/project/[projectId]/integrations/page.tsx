...
function useCreateWebhook() {
  ...
  return useMutation({
    mutationFn: async (webhook: { project_id: string; url: string; events: string[]; secret?: string }) => {
      const { data, error } = await supabase.from('webhooks').insert(webhook).select().single();
      if (error) throw error;
      return data;
    },
    onSettled: (_d, _e, vars) => {
      queryClient.invalidateQueries({ queryKey: ['webhooks', vars.project_id] });
    },
  });
}
...
export default function IntegrationsPage() {
  ...
  return (
    <div className="max-w-2xl px-6 py-4 space-y-6">
      <h1 className="text-lg font-semibold text-content-primary">Integrations</h1>

      {/* GitHub section */}
      <section className="space-y-2">
        <h2 className="text-sm font-medium text-content-primary">GitHub</h2>
        <p className="text-[12px] text-content-muted">
          Connect your GitHub repository to link pull requests and auto-transition tickets.
        </p>
        {integrations && integrations.length > 0 ? (
          <div className="space-y-1">
            {integrations.map((int) => (
              <div key={int.id} className="flex items-center justify-between border border-border-subtle rounded-md p-2">
                <span className="text-[12px] text-content-primary">{int.config?.repo ?? int.id}</span>
                <span className={`text-[10px] px-1.5 py-0.5 rounded ${int.enabled ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
          ...
          <div className="border border-dashed border-border-subtle rounded-md p-4 text-center">
            <p className="text-[12px] text-content-muted mb-2">No GitHub integration configured.</p>
      ...
      <section className="space-y-2">
        ...
        {showWebhookForm && (
          <div className="border border-border-subtle rounded-md p-3 space-y-2">
            <div>
              <label className="text-[11px] text-content-muted block mb-0.5">URL</label>
              <input
                type="url"
                value={webhookUrl}
            ...
            <div>
              <label className="text-[11px] text-content-muted block mb-0.5">Events</label>
              <div className="flex flex-wrap gap-1.5">
                {EVENT_OPTIONS.map((event) => (
                  <label key={event} className="flex items-center gap-1 cursor-pointer">
                    ...
                    />
                    <span className="text-[11px] text-content-secondary">{event}</span>
                  </label>
                ))}
              </div>
            </div>
            <div>
              <label className="text-[11px] text-content-muted block mb-0.5">Secret (optional)</label>
              <input
                type="password"
                value={webhookSecret}
        ...
        {webhooks && webhooks.length > 0 ? (
          <div className="space-y-1">
            {webhooks.map((wh: { id: string; url: string; events: string[]; is_active: boolean }) => (
              <div key={wh.id} className="flex items-center justify-between border border-border-subtle rounded-md p-2">
                <div>
                  <span className="text-[12px] text-content-primary block">{wh.url}</span>
                  <span className="text-[10px] text-content-muted">{wh.events?.join(', ')}</span>
                </div>
                <button
                  onClick={() => deleteWebhook.mutate({ id: wh.id, projectId })}
          ...
          !showWebhookForm && (
            <p className="text-[12px] text-content-muted">No webhooks configured.</p>
          )
        )}
      </section>
    </div>
