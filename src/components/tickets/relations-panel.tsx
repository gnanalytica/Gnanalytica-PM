...
import { useTicketRelations, useCreateRelation, useDeleteRelation } from '@/lib/hooks/use-relations';
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-[12px] font-medium uppercase tracking-wide text-content-muted">Relations</span>
        <button
        ...
        <div key={type} className="space-y-0.5">
          ...
          {rels?.map((r) => {
            ...
            return (
              ...
              >
                <button
                  onClick={() => linkedTicket && onTicketClick?.(linkedTicket.id)}
                  className="text-[12px] text-content-primary hover:text-accent truncate flex-1 text-left"
                >
                  {linkedTicket?.title ?? 'Unknown ticket'}
                </button>
                <button
                  onClick={() => deleteRelation.mutate({ id: r.id, ticketId })}
                  className="text-content-muted hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all text-[10px]"
                >
                  Remove
                </button>
              </div>
      ...
      {/* Add relation form */}
      {showAdd && (
        <div className="space-y-1.5 border border-border-subtle rounded p-2">
          <select
            value={selectedType}
            ...
            <div className="max-h-32 overflow-y-auto space-y-0.5">
              {filteredTickets.map((t) => (
                <button
                  ...
                  className="w-full text-left px-2 py-1 text-xs hover:bg-hover rounded transition-colors truncate text-content-secondary"
                >
                  <span className="text-content-muted font-mono mr-1">{t.id.slice(0, 6)}</span>
                  {t.title}
                </button>
              ))}
              {filteredTickets.length === 0 && (
                <p className="text-[11px] text-content-muted px-2 py-1">No matching tickets</p>
              )}
            </div>
          )}
        </div>
