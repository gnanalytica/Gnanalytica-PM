...
import { useTicketAttachments, useUploadAttachment, useDeleteAttachment } from '@/lib/hooks/use-attachments';
    ...
    <div className="space-y-2">
      ...
      >
        <p className="text-[11px] text-content-muted">
          {uploadAttachment.isPending ? 'Uploading...' : 'Drop files here or click upload'}
        </p>
      </div>
          ...
          >
            <div className="flex-shrink-0 w-6 h-6 bg-surface-secondary rounded flex items-center justify-center">
              {isImage(att.mime_type) ? (
                <svg className="w-3.5 h-3.5 text-content-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[12px] text-content-primary truncate">{att.file_name}</p>
              <p className="text-[10px] text-content-muted">{formatFileSize(att.file_size)}</p>
            </div>
            <button
              onClick={() =>
              ...
              className="text-content-muted hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all"
            >
