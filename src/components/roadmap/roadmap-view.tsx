...
function MilestoneBar({
  ...
  const startOffset = Math.max(
    0,
    (barStartDate.getTime() - earliest.getTime()) / (1000 * 60 * 60 * 24),
  );
  const duration = 60;
...
export function RoadmapView({ projectId }: { projectId: string }) {
  ...
  while (monthStart <= latest) {
    const offset =
      (monthStart.getTime() - earliest.getTime()) / (1000 * 60 * 60 * 24);
    months.push({
    ...
    <div className="space-y-2">
      ...
      <div className="border border-border-subtle rounded-md overflow-hidden">
        ...
        <div className="relative min-h-[100px] px-1 py-2">
          ...
          {milestones.length > 0 && (
            <div className="mb-2">
              <span className="text-[10px] font-medium uppercase text-content-muted px-2">
                Milestones
              </span>
              {milestones.map((m) => (
                <MilestoneBar
            ...
            <div>
              ...
              {epics.map((epic) => {
                const start = epic.start_date
                  ...
                  : new Date();
                const end = epic.due_date
                  ? new Date(epic.due_date)
