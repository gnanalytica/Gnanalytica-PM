...
export function ProjectOverview({ projectId }: { projectId: string }) {
  ...
  const stats = useMemo(() => {
    const open = tickets.filter(
      (t) =>
        t.status_category !== "completed" && t.status_category !== "canceled",
    ).length;
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
      <div className="border border-border-subtle rounded-md p-3">
        <p className="text-[11px] text-content-muted uppercase font-medium">
          Open Issues
        </p>
        <p className="text-xl font-semibold text-content-primary mt-1">
          {stats.open}
        </p>
      </div>
      <div className="border border-border-subtle rounded-md p-3">
        ...
        <p className="text-xl font-semibold text-[#5fae7e] mt-1">
          {stats.closed}
        </p>
      </div>
      <div className="border border-border-subtle rounded-md p-3">
      ...
      <div className="border border-border-subtle rounded-md p-3">
        ...
        <p
          className={`text-xl font-semibold mt-1 ${stats.overdue > 0 ? "text-[#c27070]" : "text-content-primary"}`}
        >
          {stats.overdue}
        </p>
      </div>
      {stats.upcomingMilestones.length > 0 && (
        <div className="col-span-2 md:col-span-4 border border-border-subtle rounded-md p-3">
          <p className="text-[11px] text-content-muted uppercase font-medium mb-1">
            Upcoming Milestones
          </p>
