    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={prevMonth}
            className="px-2 py-0.5 text-[11px] text-content-muted hover:text-content-secondary transition-colors"
          >
            &larr;
          </button>
      ...
      <div className="border border-border-subtle rounded-md overflow-hidden">
        ...
        <div className="grid grid-cols-7 bg-surface-secondary border-b border-border-subtle">
          {weekDays.map((d) => (
            <div key={d} className="text-center text-[10px] font-medium text-content-muted py-1.5">
              {d}
            </div>
          ))}
        </div>
            ...
            return (
              <div
                ...
                className={`min-h-[80px] border-b border-r border-border-subtle p-1 ${
                  day ? 'bg-surface-primary' : 'bg-surface-secondary'
                }`}
              >
                {day && (
                  <>
                    ...
                    <div className="mt-0.5 space-y-0.5">
                      {dayTickets.slice(0, 3).map((t) => (
                        <button
                          ...
                          className="block w-full text-left text-[10px] px-1 py-0.5 rounded truncate hover:bg-hover transition-colors"
                          style={{ borderLeft: `2px solid ${PRIORITY_COLOR[t.priority] ?? '#8b919a'}` }}
                        >
                          {t.title}
                        </button>
                      ))}
