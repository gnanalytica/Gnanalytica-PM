...
const ICONS = {
  ...
  inbox:
    "M2.25 13.5h3.86a2.25 2.25 0 0 1 2.012 1.244l.256.512a2.25 2.25 0 0 0 2.013 1.244h2.21a2.25 2.25 0 0 0 2.013-1.244l.256-.512a2.25 2.25 0 0 1 2.013-1.244h3.859M12 3v8.25m0 0-3-3m3 3 3-3",
...
export function CommandPalette() {
  ...
  const isDebouncedSearching = !page && debouncedSearch.length >= 2;
  const fuse = useSearchIndex();
  const { results: commentResults, isSearching: commentsLoading } =
    useCommentSearch(debouncedSearch, isDebouncedSearching);

  // Compute fuse results
  const fuseResults =
    isSearching && fuse ? fuse.search(search, { limit: 10 }) : [];
  const ticketResults = fuseResults
    .filter((r) => r.item.type === "ticket")
    .slice(0, 5);
  const projectResults = fuseResults
      ...
      <Command.List>
        ...
        {!page && !isSearching && (
          <>
            <Command.Group heading="Actions">
              ...
              {/* Assign Milestone */}
              <Command.Item
                onSelect={() => pushTicketAction("assign-milestone")}
              >
              ...
              {/* Go to My Issues */}
              <Command.Item
                onSelect={() => runAction(() => router.push("/my-issues"))}
              >
              ...
              >
                <Icon d={ICONS.search} />
                Focus Search
                <span className="ml-auto text-[11px] text-content-muted font-mono">
                  /
                </span>
              </Command.Item>
            </Command.Group>
                  ...
                  >
                    ...
                    <span className="truncate">
                      {item.item_type === "ticket"
                        ? (ticketById[item.item_id]?.title ??
                          item.item_id.slice(0, 8))
                        : ""}
                      {item.item_type === "project"
                        ? (projects?.find((p) => p.id === item.item_id)?.name ??
                          item.item_id.slice(0, 8))
                        : ""}
        ...
        {!page && isSearching && (
          <>
            {ticketResults.length > 0 && (
              <Command.Group heading="Issues">
                {ticketResults.map((result) => {
                  const ticket =
                    result.item.type === "ticket" ? result.item.item : null;
                  const projectName =
                    result.item.type === "ticket"
                  ...
                  return (
                    <Command.Item
                      ...
                      onSelect={() =>
                        runAction(() => router.push(`/ticket/${ticket.id}`))
                      }
                    >
                      <span
                        className="w-2.5 h-2.5 rounded-full flex-shrink-0"
                        style={{
                          backgroundColor: statusDotColor(ticket.status),
                        }}
                      />
                      <span className="truncate">{ticket.title}</span>
                      {projectName && (
                        <span className="ml-auto text-[11px] text-content-muted flex-shrink-0">
                          {projectName}
                        </span>
                      )}
                    </Command.Item>
            ...
            {projectResults.length > 0 && (
              <Command.Group heading="Projects">
                {projectResults.map((result) => {
                  const project =
                    result.item.type === "project" ? result.item.item : null;
            ...
            <Command.Group
              heading={commentsLoading ? "Comments (searching...)" : "Comments"}
            >
              ...
              {commentResults.map((comment) => (
                <Command.Item
                  ...
                  onSelect={() =>
                    runAction(() => router.push(`/ticket/${comment.ticket_id}`))
                  }
                >
        ...
        {page === "assign-user" && activeTicket && (
          <Command.Group heading={`Assign user — ${activeTicket.title}`}>
            ...
            >
              <div className="w-4 h-4 rounded-full bg-surface-tertiary flex-shrink-0" />
              Unassigned
