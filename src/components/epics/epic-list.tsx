...
export function EpicList({
  ...
  const getCompletedChildCount = (epicId: string) =>
    allTickets.filter(
      (t) => t.epic_id === epicId && t.status_category === "completed",
    ).length;

  if (epics.length === 0) {
    return (
      <p className="text-sm text-content-muted text-center py-6">
        No epics yet. Create a ticket with type &ldquo;Epic&rdquo; to organize
        related work.
      </p>
    );
  }
    ...
    <div className="space-y-2">
      ...
      {epics.map((epic) => {
        ...
        return (
          <div
            key={epic.id}
            className="border border-border-subtle rounded-md p-3 hover:bg-hover hover:shadow-xs active:scale-[0.99] transition-all duration-150 cursor-pointer"
