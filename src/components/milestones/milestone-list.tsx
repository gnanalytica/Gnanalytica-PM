...
function MilestoneProgress({
  ...
  const completed = milestoneTickets.filter(
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="px-2.5 py-1 text-[12px] bg-accent text-white rounded hover:opacity-90 hover:shadow-xs transition-all duration-150 active:scale-[0.96]"
        >
          New Milestone
        </button>
      </div>
      ...
      {milestones.length === 0 ? (
        <p className="text-sm text-content-muted text-center py-6">
          No milestones yet. Create your first milestone to organize work.
        </p>
      ) : (
        <div className="space-y-2">
          {milestones.map((m) => (
            <div
              key={m.id}
              className="border border-border-subtle rounded-md p-3 hover:bg-hover transition-colors"
            >
                ...
                <div className="flex items-center gap-1">
                  {m.status === "active" && (
                    <button
                      ...
                      className="text-[11px] text-content-muted hover:text-green-600 transition-all duration-150 active:scale-[0.96] px-1"
                    >
                      Complete
                    </button>
                  )}
                  <button
                    ...
                    className="text-[11px] text-content-muted hover:text-red-400 transition-all duration-150 active:scale-[0.96] px-1"
                  >
                    Delete
                  </button>
                </div>
              ...
              {m.description && (
                <p className="text-[12px] text-content-muted mb-1.5">
                  {m.description}
                </p>
              )}
