...
function Bone({ className }: { className?: string }) {
  return (
    <div
      className={`rounded bg-gray-200/70 animate-shimmer ${className ?? ''}`}
    />
  );
}

/** Dark variant for sidebar (gray-900 background) */
function DarkBone({ className, style }: { className?: string; style?: React.CSSProperties }) {
  return (
    <div
      className={`rounded bg-gray-700/50 animate-shimmer ${className ?? ''}`}
      style={{
        background:
          'linear-gradient(90deg, rgba(255,255,255,0) 0%, rgba(255,255,255,0.04) 50%, rgba(255,255,255,0) 100%) no-repeat rgb(55 65 81 / 0.5)',
...
// ── SidebarSkeleton ──
// Matches Sidebar: header, nav links (Dashboard, My Issues), Projects section, user section

export function SidebarSkeleton() {
  return (
    <aside className="w-60 h-screen bg-surface-secondary flex flex-col flex-shrink-0">
      {/* Header */}
      <div className="flex items-center justify-between px-4 h-12 border-b border-border-subtle">
        <DarkBone className="h-4 w-24" />
        <DarkBone className="h-5 w-5 rounded" />
      </div>
      ...
      <div className="border-t border-border-subtle px-4 py-2.5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <DarkBone className="w-6 h-6 rounded-full" />
            <DarkBone className="h-3.5 w-20" />
          </div>
          <DarkBone className="h-3 w-14" />
        </div>
      </div>
    </aside>
  );
