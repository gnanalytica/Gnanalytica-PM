...
function InboxLink({ pathname }: { pathname: string }) {
  ...
  return (
    <Link
      ...
      className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm mt-0.5 ${
        isActive
    ...
    >
      ...
      Inbox
      {unreadCount > 0 && (
        <span className="ml-auto text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-blue-600 text-white">
...
export function Sidebar() {
  ...
  return (
    <aside className="w-60 h-screen bg-surface-secondary text-content-secondary flex flex-col flex-shrink-0">
      {/* Header */}
      <div className="flex items-center justify-between px-4 h-12 border-b border-border-subtle">
        <Link href="/dashboard" className="font-medium text-content-primary text-sm">
          Internal PM
        </Link>
        <NotificationBell />
      </div>
              ...
              return (
                <div key={project.id}>
                  <Link
                    ...
                    className={`block px-3 py-1.5 rounded-md text-sm truncate transition-colors duration-[120ms] ${
      ...
      <div className="border-t border-border-subtle px-4 py-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-sm text-content-secondary truncate">{profile?.name ?? 'User'}</span>
          </div>
          <button
            onClick={signOut}
            className="text-xs text-content-muted hover:text-content-secondary flex-shrink-0 rounded transition-colors duration-[120ms] cursor-pointer"
          >
            Sign out
          </button>
        </div>
