  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Notification[]> => {
      const { data, error } = await supabase
        .from('notifications')
        .select('*, ticket:tickets(id, title), actor:profiles!notifications_actor_id_fkey(id, name, avatar_url)')
...
export function formatNotification(n: Notification): string {
  ...
  switch (n.type) {
    ...
    case 'status_changed': return `${actor} changed the status`;
    case 'priority_changed': return `${actor} changed the priority`;
